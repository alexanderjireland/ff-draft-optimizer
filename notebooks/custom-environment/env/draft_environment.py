...
class DraftEnvironment(AECEnv):
    ...
    def reset(self):
        ...
        self.team_positions_counts = {
            agent: {
                pos: 0 for pos in self.draftable_positions
            }
            for agent in self.agents
        }
    ...
    def step(self, action):
        ...
        if player and self._draft_player(agent, action, player):
            self._advance_draft(agent, action)
        else:
            self._advance_draft(agent, action=None)
    ...
    def _draft_player(self, agent, action, player):
        ...
        self.team_positions_counts[agent][player_pos] += 1
    ...
    def _advance_draft(self, action):
        ...
        self.agent_selection = self.current_agent() # Current agent now next agent since current pick has advanced
    ...
    def _get_difference_with_replacement(self):
        return [self.pos_dqs[pos].diffs[0] for pos in self.draftable_positions]
    
    def _get_top_pos_proj_pts(self):
        return [self.pos_dqs[pos].players[0][1] for pos in self.draftable_positions]
    
    def _get_team_needs(self, agent):
        needs = np.zeros(len(self.draftable_positions))
        counts = self.team_positions_counts[agent]
        ...
        excess_pos = sum([max(0, counts[pos] - self.position_limits[pos]) for pos in self.flex_positions])
    ...
    def _get_next_opponent(self, agent):
        ...
        print(f"Oh no! Couldn't find next opponent...")
        return agent
    ...
    def _get_value_over_replacement(self, action):
        return self._get_difference_with_replacement()[action]
    
    def _get_draft_pick_reward(self, agent, action):
        # Value Over Replacement + Hurt Score
        value_over_replacement = self._get_value_over_replacement(action)
